...
const SignupPage: React.FC<SignupPageProps> = ({ navigate }) => {
  ...
  const handleSignup = (e: React.FormEvent) => {
    e.preventDefault();
    if (!agreedToTerms) {
      alert('Please agree to the Terms of Service and Privacy Notice');
      return;
    }
    // Demo: Show success message
    alert('Signup functionality will be implemented with Firebase Authentication');
  };

  const handleSocialSignup = (provider: string) => {
    alert(`${provider} signup will be implemented with Firebase Authentication`);
  };
      ...
      <div className="max-w-md w-full">
        ...
        <div className="bg-gray-900/50 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-800 p-8">
          <form onSubmit={handleSignup} className="space-y-6">
            ...
            <button
              type="submit"
              className="w-full py-3.5 bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-600 hover:to-blue-700 text-white font-bold rounded-lg transition-all duration-300 transform hover:scale-[1.02] shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
          ...
          <div className="mt-8 text-center space-y-3">
            <div className="flex items-center justify-center gap-2 text-sm text-gray-400">
              <button className="text-cyan-500 hover:text-cyan-400 font-semibold transition-colors">
