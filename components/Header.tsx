...
const NavLink: React.FC<{ item: NavItem }> = ({ item }) => {
    ...
    const handleNavigation = (href?: string, isDashboard?: boolean) => {
        if (href) {
                    ...
                    <div className="space-y-1">
                        {item.children.map((child, index) => (
                            <button
                                ...
                                onClick={(e) => {
                                    e.preventDefault();
                                    handleNavigation(child.href, child.isDashboard);
                                }}
       ...
       <div className={`lg:hidden absolute top-20 left-0 w-full bg-[#0D0D0D] transition-all duration-300 ease-in-out overflow-hidden ${isMenuOpen ? 'max-h-screen border-t border-gray-800' : 'max-h-0'}`}>
        <div className="flex flex-col p-4 space-y-2">
            ...
            <div key={index} className="w-full">
              {link.children ? (
                  ...
                  <div className="pl-2">
                    {link.children.map((child, childIndex) => (
                       <button 
                         ...
                         onClick={(e) => { 
                           e.preventDefault(); 
                           if (child.href === 'ai-blogs-studio') {
                             routerNavigate('/AIBlogsStudio');
                           } else if (child.href) {
                             routerNavigate(child.href === 'home' ? '/' : `/${child.href}`);
                           } else if(child.isDashboard) {
                             routerNavigate('/dashboard');
                           }
                           setIsMenuOpen(false); 
                         }} 
                         className="block w-full text-left py-2 px-2 rounded-md text-white hover:bg-gray-500/10 hover:text-accent-yellow"
                       >
                         {child.label}
                       </button>
                    ))}
                ...
                <button 
                  onClick={(e) => { 
                    e.preventDefault(); 
                    if(link.href) routerNavigate(link.href === 'home' ? '/' : `/${link.href}`); 
                    setIsMenuOpen(false); 
                  }} 
                  className="block w-full text-left py-2 px-2 rounded-md text-white hover:bg-gray-500/10 hover:text-accent-yellow font-semibold"
                >
                  {link.label}
                </button>
              )}
          ...
          <div className="flex flex-col space-y-3 pt-4 border-t border-gray-700">
             <button 
               onClick={() => { routerNavigate('/login'); setIsMenuOpen(false); }} 
            ...
            <button 
              onClick={() => { routerNavigate('/signup'); setIsMenuOpen(false); }} 
